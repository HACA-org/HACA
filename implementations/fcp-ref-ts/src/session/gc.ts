// Session GC — compact session history after a compact-session cycle.
// Reads memory/session.jsonl, keeps the N most recent conversation turns,
// truncates tool_result content, and rewrites the file.
// Target: reduce context to ~22% of model window.
...
export async function compactSessionHistory(
  ...
  logger.info('session:gc', { originalLines, keptLines: result.length, keepCount })
