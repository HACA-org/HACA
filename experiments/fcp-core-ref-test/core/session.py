...
def _session_loop(ctx: BootContext, ui: UI, pending_proposals: list[dict]) -> None:
    ...
    while True:
        ...
        if not inbox_envs:
            try:
                line = ui.operator_input(slash_cmds)
...
def _collect_evolution_decisions(
    ...
    ui.info(f"[SIL] {len(pending_proposals)} Evolution Proposal(s) pending Operator decision:")
    for i, p in enumerate(pending_proposals, 1):
        ui.info(f"[{i}] tx={p['tx'][:8]}…")
        ui.info(p["content"][:400])
        ans = terminal_prompt(
