...
def run_boot(entity_root: str | Path) -> BootContext:
    ...
    # Warn when Google backend is configured — tool_use not supported (§3.2)
    _backend_spec = baseline.get("cpe", {}).get("backend", "")
    if _backend_spec.startswith("google"):
        print(
            "[Boot/1] WARNING: Google backend configured but does not support tool_use. "
            "Sessions will fail on the first CPE tool call."
        )
        write_notification(root, SEVERITY_DEGRADED, {
            "event":   "GOOGLE_CPE_NO_TOOLS",
            "message": "Google provider does not support FCP tool_use. All cognitive cycles will fail on the first tool call.",
        })
    ...
    # [SKILLS INDEX] — all skills except operator_only (§9.5)
    ...
    if skills_index_path.exists():
        ...
        visible = [s for s in index_data.get("skills", []) if not s.get("operator_only")]
        if visible:
